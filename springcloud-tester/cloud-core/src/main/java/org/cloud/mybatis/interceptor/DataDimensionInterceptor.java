...
public class DataDimensionInterceptor implements Interceptor {
  ...
  private String generateDataDimensionSql(String boundSql) {
      ...
      if (dataDimensionValueSet == null || dataDimensionValueSet.size() == 0) {
        ...
        if (dataDimensionValueSet.contains("***")) {  // 如果包含了***,那么说明有全部仅限
          ...
          if ("in".equalsIgnoreCase(dataDimension.getOperator())) {
            String mergeString = dataDimensionValueSet.stream().filter(string -> !string.isEmpty()).collect(Collectors.joining("','"));
            result.append(" (" + dataDimension.getFieldName() + " " + dataDimension.getOperator() + " " + "('" + mergeString + "'))");
          } else {
            int idx1 = 0;
            Iterator<String> dataDimensionIterator = dataDimensionValueSet.iterator();
            while (dataDimensionIterator.hasNext()) {
              result.append(
                  " (" + dataDimension.getFieldName() + " " + dataDimension.getOperator() + " " + "'" + dataDimensionIterator.next()
                      + "')");
              idx1++;
