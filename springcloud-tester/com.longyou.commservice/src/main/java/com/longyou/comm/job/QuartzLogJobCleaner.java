...
@Component
public class QuartzLogJobCleaner extends BaseQuartzJobBean {

    Logger logger = LoggerFactory.getLogger(QuartzLogJobCleaner.class);

    @Override
    protected void init() {
        this.jobName = "定时清除quartz定时任务日志";
        jobData.put("description", "定时清除quartz定时任务日志,默认每晚23:59分执行");
    ...
    protected void executeInternal(@NotNull JobExecutionContext context) {
        ...
        try {
            Query query = new Query(
                Criteria.where(MongoDbLogConfig.CREATE_DATE_FIELD.value()).lt(new Date(System.currentTimeMillis() - expireDays * 24 * 60 * 60 * 1000)));
